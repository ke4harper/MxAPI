	// Shared memory
	{
		...
		id = mca_Crc32_ComputeBuf(id,"mrapi_implt_shmem",17) + d_offset;
		...
		assert((int32_t)NULL != mrapi_db->shmems[m_index].id);	// Internal handle
		assert(MRAPI_TRUE == mrapi_db->shmems[m_index].valid);
		assert(NULL == mrapi_db->shmems[m_index].addr);
		...
		assert(mrapi_impl_shmem_exists(id));
		assert(mrapi_impl_shmem_get(&shmem2,id));
		assert(shmem2 == shmem1);

		// Shared memory access
		addr = mrapi_impl_shmem_attach(shmem1);
		assert(NULL != addr);
		assert(addr == mrapi_db->shmems[m_index].addr);
