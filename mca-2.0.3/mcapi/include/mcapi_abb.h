...
enum mcapi_status_codes {
	...
	MCAPI_ERR_NODE_CONFFAILED,  	// The MCAPI node endpoints or channels could not be configured
	MCAPI_ERR_NODE_FINALFAILED,		// (10) The MCAPI could not be finalized
  ...
  MCAPI_ERR_ENDP_LIMIT,
	MCAPI_ERR_ATTR_INCOMPATIBLE,	// (20) Connection of endpoints with incompatible attributes not allowed
	...
	MCAPI_ERR_CHAN_DIRECTION,		// (30) Attempt to open a send handle on a port that was connected as a receiver, or vice versa
	...
	MCAPI_ERR_MEM_LIMIT,			// (40) Out of memory
    ...
    MCAPI_ERR_QUEUE_FULL,           // Endpoint queue cannot accept entries
    MCAPI_ERR_QUEUE_EMPTY,          // Endpoint queue has no entries
    MCAPI_ERR_QUEUE_FULL_CONSUMER_READING, // Immediate push retry should succeed
    MCAPI_ERR_QUEUE_EMPTY_PRODUCER_INSERTING, // Immediate pop retry should succeed
        MCAPI_ERR_GENERAL,
	MCAPI_STATUSCODE_END			// (50) This should always be last
};
...
/* Buffer Types */
typedef enum mcapi_buffer_type {
	MCAPI_ENDP_ATTR_FIFO_BUFFER,			/* Event message communication (Default) */
	MCAPI_ENDP_ATTR_STATE_BUFFER			/* State message communication */
} mcapi_buffer_type;
...
enum mcapi_memory_type {
	...
	MCAPI_ENDP_ATTR_REMOTE_MEMORY       /* In anticipation of MRAPI remote memory integration */
};
...
/*******************************************************************
 The message configuration database
*******************************************************************/

/* Links are compositions of channels */
typedef enum {
  MCAPI_FULLDUPLEX_LINK
} mcapi_link_type_t;

/* A channel has a direction, based on the referenced endpoint */
typedef enum {
  MCAPI_SEND_DIRECTION,
  MCAPI_RECV_DIRECTION
} mcapi_direction_t;

/* The lifecycle of a channel goes through an number of states */
typedef enum {
  MCAPI_INVALID_STATE,
  MCAPI_CONN_STATE,
  MCAPI_ACTIVE_STATE,
...
typedef struct {
  ...
  int valid;
  channel_type type;
  mcapi_buffer_type buffer_type;
  mcapi_uint_t buffer_length;
  uintptr_t index;
  void* parent;
...
/* Link configuration bound to its channels */
typedef struct {
  char name[MAX_CFG_NAME_LEN+1];
...
PUBLIC
extern void mcapi_node_rundown(
	MCAPI_OUT mcapi_status_t* mcapi_status
);
